...
export default function Monitor404Page() {
  const { data, loading, error } = useGscHealth("404_urls");
  ...
  const urls = data.map((r) => ({
    ...
    lastCrawled: String(r.data.lastCrawled || r.report_date),
  }));
          ...
          <div className="space-y-2">
            {topPatterns.map(([pattern, count]) => (
              <div key={pattern} className="flex items-center gap-3">
                ...
                <span className="text-gray-300 text-sm w-32 truncate">{pattern}</span>
